...
import {addUser} from '../../formReducer/user'
...
function checkDateValid(dob){
   ...
   if (date.length === 1){
       date = dob.split('-')
        givenDate = moment(`${dob}`,"DD-MM-YYYY",true).isValid()
   }else{
      givenDate = moment(`${dob}`,"DD/MM/YYYY",true).isValid()
   }
...
const Form = (props) => {
    ...
    const dataList = useSelector((state)=>state.user.value)
    ...
    const onSubmitForm = (event) =>{
        ...
        if ( userName.name!=="" && userEmail.email!=="" && userDob.dob!=="") {
            ...
            if (dataList === null && isNameCorrect===true  && isEmailCorrect===true && isDobCorrect=== true){
                dispatch(addUser(newFormData)) 
                        navigate('/')
            }else if (isNameCorrect===true  && isEmailCorrect===true && isDobCorrect=== true){
                const isEmailAlreadyExist = dataList.find(eachData=>eachData.email===userEmail.email)
